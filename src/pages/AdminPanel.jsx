...
const AdminPanel = () => {
  ...
  const handleSave = async (userId) => {
    setIsUpdating(true);
    try {
      await axios.put(`http://localhost:5001/api/auth/users/${userId}`, editFormData);
      setEditingId(null);
      fetchUsers();
    } catch (err) {
      console.error(err);
                ...
                <tr key={u._id} className="border-b border-borderC hover:bg-gray-50 transition-colors">
                  <td className="p-3 font-medium text-textMain flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold overflow-hidden flex-shrink-0">
                      {u.profilePicture ? <img src={`http://localhost:5001${u.profilePicture}`} className="w-full h-full object-cover"/> : u.name.charAt(0).toUpperCase()}
                    </div>
